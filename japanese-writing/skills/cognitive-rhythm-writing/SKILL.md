...
description: 説明的な文章に緩急を設計するための規範。緩急を装飾ではなく認知モードの切替（観察→逡巡→断定→再観察）と未回収の緊張の管理として扱い、文の拍、段落の密度波形、節の入り方、緩みと駄文の判別、執筆後の機械的な点検手順を定める。読み物として読ませたい章・記事・解説文（技術記事、設計ドキュメントの背景・導入、スパイク報告、スライド草稿、ブログ草稿など）を生成・推敲するとき、または「密度はあるが平坦でおもしろくない」「情報は正しいが読み進める気が起きない」文章を診断・修正するときに使用する。日本語の技術文書一般の文章規範 japanese-tech-writing と併用する（読み物性・推進力を足す層）。
---
